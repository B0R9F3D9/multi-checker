...
export function getChainsComponent(
	chains: LayerzeroWallet['srcChains'] | LayerzeroWallet['dstChains'],
) {
	if (chains?.length === 0) {
					...
					<div className="flex flex-row items-center gap-0 overflow-x-auto max-w-full">
						{chains!.map(chain => (
							<div key={chain.name} className="w-6 h-6 flex-shrink-0">
								<Image
									src={`https://icons-ckg.pages.dev/lz-scan/networks/${chain.name}.svg`}
									className="rounded-full"
									alt={chain.name}
									width={24}
									height={24}
									title={chain.name.replace(/^\w/, c => c.toUpperCase())}
								/>
							</div>
						))}
					</div>
